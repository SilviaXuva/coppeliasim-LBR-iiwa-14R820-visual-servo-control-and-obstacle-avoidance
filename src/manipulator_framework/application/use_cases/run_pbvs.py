...
class RunPBVS:
    """
    Application use case for PBVS execution.
    """
    ...
    def execute(self, request: RunPBVSRequest) -> RunResponse:
        ...
        run_result = replace(
            ...
            summary={
                **run_result.summary,
                "experiment_name": "run_pbvs",
                "scenario_name": request.config.get("scenario_name", "pbvs_synthetic_target"),
                "backend_name": request.config.get("backend_name", "mock"),
                "pipeline_stages": ("sensing", "planning", "control", "actuation"),
            },
        )
